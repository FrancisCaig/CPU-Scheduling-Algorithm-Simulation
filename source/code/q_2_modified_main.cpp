...
//            caig@rpi.edu  katsig@rpi.edu     qianf@rpi.edu
//
...
void statistic(const list<Process>  &result, int cont, ofstream &outfile)   //get average of burst for every algo times to report
{
...
void FCFS(vector<Process> storage, int cont, ofstream &outfile) // try to check every millisecond. should use const or & (cuz we erase elements at end of loop)
{
    ...
    statistic(result, cont, outfile);

}

// Function to simulate the SJF algorithm
void SJF(vector<Process> storage, int cont, ofstream &outfile)
{
    // report processes at begin
    for (auto i: storage)
    {
...
void SRT(vector<Process> storage, int cont, ofstream &outfile)
{
    // report processes at begin
    for (auto i: storage)
    {
...
void RR(vector<Process> storage, int cont, int slice, string order, ofstream &outfile)
{
...
int main(int argc, char* argv[]) {
    ...
    // added to run Q2 and Q3 experiments
    string filename;
    if (argc >= 10)
    {
        filename = argv[9];
    ...
    FCFS(storage, cont, outfile);
    cout<<endl;

    SJF(storage, cont, outfile);
    cout<<endl;

    SRT(storage, cont, outfile);
    cout<<endl;

    RR(storage, cont, slice, order, outfile);
